...
class Grades:
    ...
    def __init__(self, projectName, questionsAndMaxesList,
                 gsOutput=False, edxOutput=False, muteOutput=False, isDisplay=True):
        """
...
""" % (prereq, q, q, prereq))
            ...
            try:
                if self.isDisplay:
                    util.TimeoutFunction(getattr(gradingModule, q), 3600)(
                        self)  # Call the question's function
                elif q == 'q5' or q == 'q1':
                    util.TimeoutFunction(getattr(gradingModule, q), 10*60 + 100)(
                        self)  # Call the question's function
                elif q == 'q6':
                    util.TimeoutFunction(getattr(gradingModule, q), 10*180 + 100)(
                        self)  # Call the question's function
                else:
                    util.TimeoutFunction(getattr(gradingModule, q), 60)(
                        self)  # Call the question's function
...
run 'python submit.py' and submit 'multiagent.tar' to autolab to receive
